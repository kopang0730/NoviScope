...
from noviscope.core.stage_policy import PAPER_MEETING_WRITER_AGENT_ID, stage_confidence
...
def human_review_state(stage: StageCard) -> str:
...
def append_quest_metadata(lines: list[str], quest: Quest, stages: list[StageCard]) -> None:
...
def append_stage_trace(lines: list[str], stages: list[StageCard]) -> None:
    ...
    for index, stage in enumerate(stages, start=1):
        ...
        append_field(lines, "Confidence", label_from_value(confidence))
...
def append_paper_artifacts(lines: list[str], stages: list[StageCard]) -> None:
    paper_stage = next(
...
def build_quest_review_packet(quest: Quest, stages: list[StageCard]) -> str:
    lines = [
...
def download_quest_review_packet(
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": (
                f'attachment; filename="{quest_review_packet_filename(quest)}"'
            )
        },
