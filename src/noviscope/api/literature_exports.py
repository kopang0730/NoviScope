...
def read_literature_papers(stage: StageCard) -> list[JsonObject]:
    value: JsonValue | None = stage.output_payload.get("papers")
    match value:
        case list() as values:
...
def build_literature_citations_markdown(stage: StageCard) -> str:
    papers = read_literature_papers(stage)
    lines = [
    ...
    append_field(lines, "Source", read_string(stage.output_payload, "source"))
    append_field(lines, "Search query", read_string(stage.output_payload, "search_query"))
    append_field(lines, "Score basis", read_string(stage.output_payload, "score_basis"))
    lines.extend(["", "## Papers", ""])
    if not papers:
