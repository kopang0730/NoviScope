...
export function buildStageDetails(stage: StageCard, t: Translate): CanvasDetail[] {
  ...
  if (stage.agent_id === paperMeetingWriterAgentId) {
    ...
    details.push({
      ...
      value: firstDetailValue(readStringArray(stage.output_payload, "model_generated_hypotheses"), fallback),
    });
    details.push({
      label: t("experimentResultsNotAvailable"),
      value: firstDetailValue(readStringArray(stage.output_payload, "experiment_results_not_available"), fallback),
    });
    details.push({
