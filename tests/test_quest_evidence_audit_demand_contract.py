...
def test_evidence_audit_rejects_mixed_valid_and_malformed_demand_sources(
    tmp_path,
    dev_admin_header_enabled: None,
) -> None:
    database_url = f"sqlite:///{tmp_path / 'audit-mixed-demand-sources.db'}"
    app = create_app(database_url=database_url)
    with TestClient(app) as client:
        register_and_login(client, "AUDIT-MIXED-DEMAND", "audit-mixed-demand@example.com")
        quest_id, stage_ids = create_quest_with_stage_map(client)
        complete_research_gates(
            client,
            stage_ids,
            ...
            json={
                "evidence_payload": {
                    "human_demand_sources": [
                        "Coach interview: shuttle training review",
                        None,
                    ],
                    "human_demand_verdict": "verified",
                }
