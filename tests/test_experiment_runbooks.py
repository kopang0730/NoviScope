...
def experiment_plan_output() -> JsonObject:
    return {
        ...
        "raw_response": "hidden raw planner response",
    ...
    complete_response = client.patch(
        ...
        json={
            "evidence_payload": {
                ...
                "plan_only": True,
                "raw_response": "hidden evidence payload",
...
def test_download_experiment_runbook_returns_plan_only_attachment(
    ...
    # Given: an owner has a completed Experiment Planner stage.
    ...
    # Then: the Markdown attachment preserves plan boundaries and hides raw fields.
    ...
    assert NO_EXPERIMENT_VERIFICATION_RISK in response.text
    assert "Hidden Payload Fields" in response.text
    assert "api_key" not in response.text
    assert "raw_response" not in response.text
    assert "hidden-input-token" not in response.text
    assert "hidden raw planner response" not in response.text
...
def test_download_experiment_runbook_blocks_until_stage_complete(
    ...
    # Given: an owner has an incomplete Experiment Planner stage.
